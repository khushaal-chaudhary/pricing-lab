...
- v4 — Poisson FE on the top-500 items — actually has the **highest R² (0.62)** in the leaderboard. But it only fits 500 of 3,000 items (coverage 0.17 vs v9's 1.00), which costs it ~0.16 on the composite. High-R² *on a curated subset* is not the same as a deliverable that covers the portfolio.
...
- v9 — MMM decomposition — R² 0.56 with **full 3,000-item coverage**, tightest bootstrap CI (0.07 vs v7's 0.20), and a transparent linear partial-out that **isolates the price signal from baseline / trend / season / event variance**. That's what makes it defensible.
